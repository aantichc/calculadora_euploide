...
# Main app
    ...
    st.write("DataFrame Columns:", df.columns.tolist())  # Display columns for debugging

    # Ensure 'Edad' column exists
    if 'Edad' in df.columns:
        edad = st.selectbox("Age", df['Edad'].tolist())
    else:
        st.error("Column 'Edad' not found in the DataFrame. Available columns: " + ", ".join(df.columns.tolist()))
        st.stop()
